...
// KNXDevice is not the accessory itself, it creates/holds a PlatformAccessory instance.
...
class KNXDevice {
	...
	constructor (globs, config, platformAccessory) {
		...
		// If the accessory was restored by Homebridge, we already have an instance.
		...
		for (var int = 0; int < currServices.length; int++) {
			...
			if (myKNXService.failed) {
				...
				this.globs.errorlog("homebridge-knx couldn't create KNX service:" + configService.ServiceName);
			} else {
