...
var readQueue = [];
var readQueueIndex = {};
var readQueueTimer = null;
var readQueueStarted = false;
var knxReadConnection = null;
var knxReadConnectionReady = false;
...
allFunctions.knxwrite = function (callback, groupAddress, dpt, value) {
    ...
    const useKnxJs = globs.knxconnection === 'knxjs';
    if (useKnxJs) {
        // use KNX JS for KNX IP
        var connection = knx.Connection({
            physAddr: globs.knx_phy_addr || "15.15.15",
            handlers: {
                connected: function () {
...
allFunctions.knxread = function (groupAddress) {
    ...
    const useKnxJs = globs.knxconnection === 'knxjs';
    if (useKnxJs) {
        if (!knxReadConnection) {
            knxReadConnection = knx.Connection({
                physAddr: globs.knx_phy_addr || "15.15.15",
                handlers: {
                    connected: function () {
                        knxReadConnectionReady = true;
                        knxReadConnection.read(groupAddress);
                    },
                    error: function () {
                        knxReadConnectionReady = false;
                    },
                    close: function () {
                        knxReadConnectionReady = false;
                        knxReadConnection = null;
                    }
                }
            });
        } else if (knxReadConnectionReady) {
            knxReadConnection.read(groupAddress);
        }
    } else {
...
allFunctions.knxreadhash = function (groupAddresses) {
    ...
    for (var address in groupAddresses) {
        if (groupAddresses.hasOwnProperty(address)) { // do not use inherited properties
            var cleanedAddress = address.match(/(\d*\/\d*\/\d*)/)[0];
            if (!readQueueIndex[cleanedAddress]) {
                readQueue.push(cleanedAddress);
                readQueueIndex[cleanedAddress] = true;
            }
        }
    }
...
function processReadQueue() {
    ...
    var nextAddress = readQueue.shift();
    delete readQueueIndex[nextAddress];
    allFunctions.knxread(nextAddress);
    readQueueTimer = setTimeout(processReadQueue, globs.readRequestDelayMs || 80);
}
...
function startReadQueue() {
    ...
    readQueueStarted = true;
    readQueueTimer = setTimeout(processReadQueue, globs.readRequestStartupDelayMs || 3000);
}
    ...
    switch (characteristic.props.format) {
        ...
        case Formats.FLOAT:
            ...
            if (validValue) {
                ...
                globs.debug("FLOAT update: " + characteristic.displayName + " = " + hk_value);
