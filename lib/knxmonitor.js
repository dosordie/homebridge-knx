...
var registerSingleGA = function registerSingleGA (groupAddress, dpt, callback) {
	...
	if (useKnxJs) {
		var dp = new knx.Datapoint({ ga: groupAddress, dpt: dpt, autoread: false });
		dp.on('event', function (event, value, src) {
			if (event !== 'GroupValue_Write' && event !== 'GroupValue_Response') {
				globs.debug("Ignoring KNX datapoint event " + event + " for " + groupAddress);
				return;
			}

			if (value === undefined || value === null || (typeof value === "number" && Number.isNaN(value))) {
				globs.debug("Ignoring invalid KNX datapoint value for " + groupAddress + ": " + value);
				return;
			}

			callback(value, src, groupAddress);
		});
