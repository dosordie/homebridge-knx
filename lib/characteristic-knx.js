...
function getCharacteristicConstant(Characteristic, category, key, fallback) {
    var group = Characteristic && Characteristic[category];
...
class CharacteristicKNX {
    ...
    constructor(service, config, globs) {
        ...
        this.charFormats = {
            BOOL: getCharacteristicConstant(this.availableCharacteristics, 'Formats', 'BOOL', 'bool'),
            INT: getCharacteristicConstant(this.availableCharacteristics, 'Formats', 'INT', 'int'),
            UINT8: getCharacteristicConstant(this.availableCharacteristics, 'Formats', 'UINT8', 'uint8'),
            PERCENTAGE: getCharacteristicConstant(this.availableCharacteristics, 'Formats', 'PERCENTAGE', 'percentage'),
            FLOAT: getCharacteristicConstant(this.availableCharacteristics, 'Formats', 'FLOAT', 'float')
        };
        this.charUnits = {
            PERCENTAGE: getCharacteristicConstant(this.availableCharacteristics, 'Units', 'PERCENTAGE', 'percentage')
        };
        this.charPerms = {
            WRITE: getCharacteristicConstant(this.availableCharacteristics, 'Perms', 'WRITE', 'pw'),
            READ: getCharacteristicConstant(this.availableCharacteristics, 'Perms', 'READ', 'pr')
        };
        if (!this.config.Type) {
        ...
        if (this.chr.props.format === this.charFormats.UINT8 || this.chr.props.format === this.charFormats.INT) {
        ...
        if (this.chr.props.perms.indexOf(this.charPerms.WRITE) > -1) {
            if (this.config.Set) {
        ...
        if (this.chr.props.perms.indexOf(this.charPerms.READ) > -1) {
    ...
    getDPT() {
        ...
        if (this.config.DPT) {
            ...
            if (this.chr.props.format === this.charFormats.BOOL) {
                // found boolean, assume DPT1
                return "DPT1";
            }
            if (this.chr.props.format === this.charFormats.INT) {
                // found INT, assume DPT5, unless props.unit is Units.PERCENTAGE
                if (this.chr.props.unit === this.charUnits.PERCENTAGE) {
                    return "DPT5.001";
                } else {
                    return "DPT5";
                }
            }
            if (this.chr.props.format === this.charFormats.UINT8) {
            ...
            if (this.chr.props.format === this.charFormats.PERCENTAGE) {
                // found PERC, assume DPT5.001
                return "DPT5.001";
            }
            if (this.chr.props.format === this.charFormats.FLOAT) {
