...
class ServiceKNX {
	...
	loadServiceData (platformAccessory) {
		...
		const NameCharacteristic = this.globs.Characteristic && this.globs.Characteristic.Name;
		const ConfiguredNameCharacteristic = this.globs.Characteristic && this.globs.Characteristic.ConfiguredName;

		if (NameCharacteristic) {
			this.serviceHK.updateCharacteristic(NameCharacteristic, this.name);
		}

		if (ConfiguredNameCharacteristic && typeof this.serviceHK.testCharacteristic === "function" &&
			this.serviceHK.testCharacteristic(ConfiguredNameCharacteristic)) {
			try {
				this.serviceHK.updateCharacteristic(ConfiguredNameCharacteristic, this.name);
			} catch (err) {
