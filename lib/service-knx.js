...
class ServiceKNX {
	...
	loadServiceData (platformAccessory) {
		...
		if (!this.config.subtype) {
			...
			Object.defineProperty(this.config, '_knxGeneratedConfigFields', {
				value: ["subtype"],
				enumerable: false,
				configurable: true
			});
		...
		// try to find it.
		/** @type {Service} */
		this.serviceHK = platformAccessory.getServiceById(this.Services[this.config.ServiceType], this.config.subtype);
		...
		// ensure HomeKit service display names always match KNX ServiceName
		const Characteristic = this.globs.Characteristic;
		const NameCharacteristic = Characteristic && Characteristic.Name;
		const ConfiguredNameCharacteristic = Characteristic && Characteristic.ConfiguredName;

		if (NameCharacteristic) {
			try {
				this.serviceHK.setCharacteristic(NameCharacteristic, this.name);
			} catch (err) {
				this.globs.info("Could not set Name for service " + this.name + ": " + err.message);
			}
		}

		if (ConfiguredNameCharacteristic) {
			try {
				if (typeof this.serviceHK.testCharacteristic === "function" &&
					!this.serviceHK.testCharacteristic(ConfiguredNameCharacteristic) &&
					typeof this.serviceHK.addOptionalCharacteristic === "function") {
					this.serviceHK.addOptionalCharacteristic(ConfiguredNameCharacteristic);
				}

				this.serviceHK.setCharacteristic(ConfiguredNameCharacteristic, this.name);
			} catch (err) {
				this.globs.info("Could not set ConfiguredName for service " + this.name + ": " + err.message);
			}
		}
