		...
		if (field==="KNXDoorClosed") {
			...
			if (knxValueNorm === this.sensorOn) {
				...
				if (oldValueNorm === this.sensorOn){ // oldValue = sensorOn
					...
					let doorIsAlreadyMoving = myCurrentDoorState === 2 || myCurrentDoorState === 3;

					if (this.doorContactPresent === "closed" && !doorIsAlreadyMoving) {
						console.log('INFO ' + this.debugName + ': KNXDoorClosed changed from closed to not closed, assuming external OPENING movement');
						console.log('INFO ' + this.debugName + ': TargetDoorState = 0 OPEN set');
						this.myAPI.setValue("TargetDoorState", 0); // OPEN
						console.log('INFO ' + this.debugName + ': CurrentDoorState = 2 OPENING set');
						this.myAPI.setValue("CurrentDoorState", 2); // OPENING
						...
						this.lastMove = "goingUp";
						console.log('INFO ' + this.debugName + ': lastMove = ' + this.lastMove+ ' set');
					} else if (doorIsAlreadyMoving) {
						console.log('INFO ' + this.debugName + ': Door is already moving, keeping CurrentDoorState = ' + myCurrentDoorState);
					}
