...
function normalizeSensorOn(value) {
...
class GarageDoorOpenerAdvanced extends HandlerPattern {
	...
	constructor(knxAPI) {
		...
		// Report OPEN immediately when KNXDoorClosed changes from active to inactive.
		this.NotifyOpenOnClosedContact = undefined;
	...
	onKNXValueChange(field, oldValue, knxValue) {
		...
		// get the sensor value for "On"
		this.sensorOn = normalizeSensorOn(this.myAPI.getLocalConstant("sensorOn"));

		// Optionally report OPEN immediately when the closed contact opens.
		this.NotifyOpenOnClosedContact = this.myAPI.getLocalConstant("NotifyOpenOnClosedContact");

		// set debugName
		this.debugName = this.myAPI.getLocalConstant("debugName") || "GarageDoorOpenerAdvanced";
		...
		const knxValueNorm = normalizeSensorOn(knxValue);
		const oldValueNorm = oldValue === null ? null : normalizeSensorOn(oldValue);
		...
		if (field==="KNXDoorClosed") {
			...
			if (knxValueNorm === this.sensorOn) {
				...
				console.log('INFO ' + this.debugName + ': lastMove = ' + this.lastMove+ ' set');
			...
			else if (knxValueNorm !== this.sensorOn) {

					if (oldValueNorm === this.sensorOn){ // oldValue = sensorOn

						console.log('INFO ' + this.debugName + ': KNXDoorClosed = false detected');

						if (this.doorContactPresent === "closed" && this.NotifyOpenOnClosedContact === true) {
							console.log('INFO ' + this.debugName + ': NotifyOpenOnClosedContact active, setting OPEN immediately');
							this.myAPI.setValue("TargetDoorState", 0); // OPEN
							this.myAPI.setValue("CurrentDoorState", 0); // OPEN
							this.percentageOpen = 1;
							this.dateRunStart = undefined;
							...
							let nextMovingState;
							let myTargetDoorState = this.myAPI.getValue("TargetDoorState");
							let myCurrentDoorState = this.myAPI.getValue("CurrentDoorState");
							if (myTargetDoorState === 1 || this.lastMove === "Closing") {
								nextMovingState = 3; // CLOSING
							} else if (myTargetDoorState === 0 || this.lastMove === "Opening") {
								nextMovingState = 2; // OPENING
							} else if (myCurrentDoorState === 1) {
								nextMovingState = 2; // leaving CLOSED most likely means OPENING
							}
							if (nextMovingState !== undefined && myCurrentDoorState !== nextMovingState) {
								console.log('INFO ' + this.debugName + ': CurrentDoorState = ' + nextMovingState + ' set while door contact is false');
								this.myAPI.setValue("CurrentDoorState", nextMovingState);
							}
							...
							if (!this.timer1) {
						if (this.simulateDoorContactMode==="knx" && this.doorContactPresent==="closed"){
							// simulate the "open" contact on knx
							...
							this.timer1 = setTimeout (function () {
								console.log('INFO ' + this.debugName + ': Timeout, checking KNXDoorClosed before setting OPEN');
								const currentClosedContact = this.myAPI.getValue("KNXDoorClosed");
								const currentClosedContactNorm = normalizeSensorOn(currentClosedContact);
								if (currentClosedContactNorm !== this.sensorOn) {
									console.log('INFO ' + this.debugName + ': KNXDoorOpened = ' + this.sensorOn + ' set');
									this.myAPI.knxWrite("KNXDoorOpened", this.sensorOn, "DPT1"); // Door open
									console.log('INFO ' + this.debugName + ': CurrentDoorState = 0 OPEN set');
									this.myAPI.setValue("CurrentDoorState", 0); // OPEN
									...
									console.log('INFO ' + this.debugName + ': Timeout but KNXDoorClosed is closed again, not setting OPEN');
								}
								this.timer1 = undefined;
							}.bind(this),this.doorRunTime);

						} else if (this.simulateDoorContactMode==="internal" && this.doorContactPresent==="closed"){
							// simulate the "open" contact internally
						...
						} else if (this.doorContactPresent === "closed") {
							console.log('INFO ' + this.debugName + ': Timer for delayed OPEN state started, timer to expire in = ' + this.doorRunTime);
							this.timer1 = setTimeout(function () {
								const currentClosedContact = this.myAPI.getValue("KNXDoorClosed");
								const currentClosedContactNorm = normalizeSensorOn(currentClosedContact);
								if (currentClosedContactNorm !== this.sensorOn) {
									console.log('INFO ' + this.debugName + ': Timeout and KNXDoorClosed still false, setting OPEN');
									this.myAPI.setValue("CurrentDoorState", 0); // Open
									this.myAPI.setValue("TargetDoorState", 0); // Open
									this.percentageOpen = 1;
									this.dateRunStart = undefined;
					...
					// On init with only KNXDoorClosed=false we do not know if the door is already open or moving.
					// Avoid reporting OPEN immediately to prevent false notifications.
					this.myAPI.setValue("CurrentDoorState", 4); // STOPPED
					this.percentageOpen = 1;
			...
			if (newValue === 0){ // OPEN
				...
				if (this.separatePulseUpDown) {
					if (myCurrentDoorState !== 0){ // not equal open
						this.sendPulseUp();
						this.setOpeningStateFromCommand();
					} else {
			...
			if (newValue === 1){ // CLOSE
				...
				if (this.separatePulseUpDown) {
					if (myCurrentDoorState !== 1){ // not equal CLOSED
						this.sendPulseDown();
						this.setClosingStateFromCommand();
					} else {
	...
	/****
	 * Set HomeKit state immediately after sending an actual open command.
	 */
	setOpeningStateFromCommand() {
		console.log('INFO ' + this.debugName + ': TargetDoorState = 0 OPEN set');
		this.myAPI.setValue("TargetDoorState", 0); // OPEN
		console.log('INFO ' + this.debugName + ': CurrentDoorState = 2 OPENING set');
		this.myAPI.setValue("CurrentDoorState", 2); // OPENING
	...
	/****
	 * Set HomeKit state immediately after sending an actual close command.
	 */
	setClosingStateFromCommand() {
		console.log('INFO ' + this.debugName + ': TargetDoorState = 1 CLOSED set');
		this.myAPI.setValue("TargetDoorState", 1); // CLOSED
		console.log('INFO ' + this.debugName + ': CurrentDoorState = 3 CLOSING set');
		this.myAPI.setValue("CurrentDoorState", 3); // CLOSING
            ...
            "Services": [
                {
                    ...
                    "LocalConstants": {
                        ...
                        "sensorOn" : 1,
                        "NotifyOpenOnClosedContact": true,
                        "pulseLength": 500, 
                        "doorRunTime": 18500
                    }
                }
