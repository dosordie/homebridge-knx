...
class WindowCoveringTilt extends HandlerPattern {
	...
	onKNXValueChange(field, oldValue, knxValue) {
		...
		if (field === "TargetPosition") {
			...
			if (newValue > this.myAPI.getValue("CurrentPosition")) {
				// newValue is higher, shutter is opening (increasing)
				this.myAPI.setValue("PositionState", 1);

			} else if (newValue < this.myAPI.getValue("CurrentPosition")){
				// newValue is lower, shutter is closing (decreasing)
			...
			switch (knxValue) {
				case 0:
					...
					this.myAPI.setValue("PositionState", 1); // opening
					break;

				case 1:
					this.myAPI.setValue("TargetPosition", 0); // low position, so home shows "closing"
					this.myAPI.setValue("PositionState", 0); // closing
					break;
	...
	onHKValueChange(field, oldValue, newValue) {
		...
		if (field === "TargetPosition") {
			...
			// is the Shutter one with a full rotation or one with 90° only?
			if (ninetyDegRotation === true) {		
				knxValue = Math.max(0, Math.min(100, newValue / 90 * 100));
			} else {
				knxValue = Math.max(0, Math.min(100, (newValue + 90) / 180 * 100));
			}
			
			console.log('INFO: onHKValueChange after calc ('  + knxValue+ ")");
			this.myAPI.knxWrite("TargetHorizontalTiltAngle", knxValue, "DPT5.001"); // send the new position to the KNX bus
